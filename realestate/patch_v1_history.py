...
Only data plumbing, listing-link canonicalization and price-change safety are
patched. The v1 layout, its all-property canvas, price-drop cards, desktop
table, mobile cards and detail sheet remain intact.
"""
...
old_link = "function linkHtml(x,label='物件を開く'){return x.url?`<a class=\"open-btn\" href=\"${esc(x.url)}\" target=\"_blank\" rel=\"noopener noreferrer\" onclick=\"event.stopPropagation()\">${label}</a>`:'<span class=\"muted\">リンクなし</span>'}"
malformed_link = "function listingUrl(x){let raw=String(x.url||'').trim();if(!raw)return'';if(sourceLabel(x.source)==='SUUMO'){try{let u=new URL(raw),id=(u.pathname.match(/\\/nc_(\\d+)\\/?$/i)||[])[1],scope=(u.pathname.match(/\\/sc_(shinagawa|meguro)\\//i)||[])[1];if(id){scope=scope||(x.ward==='目黒区'?'meguro':'shinagawa');return`https://suumo.jp/chukoikkodate/tokyo/sc_${scope}/nc_${id}/`}}catch(_){let id=(raw.match(/nc_(\\d+)/i)||[])[1];if(id)return`https://suumo.jp/chukoikkodate/tokyo/sc_${x.ward==='目黒区'?'meguro':'shinagawa'}/nc_${id}/`}}return raw.replace(/[?#].*$/,'').replace(/\\/+$/,'')+'/'}return raw}function linkHtml(x,label='物件を開く'){let url=listingUrl(x);return url?`<a class=\"open-btn\" href=\"${esc(url)}\" target=\"_blank\" rel=\"noopener noreferrer\" onclick=\"event.stopPropagation()\">${label}</a>`:'<span class=\"muted\">リンクなし</span>'}"
new_link = "function listingUrl(x){let raw=String(x.url||'').trim();if(!raw)return'';if(sourceLabel(x.source)==='SUUMO'){try{let u=new URL(raw),id=(u.pathname.match(/\\/nc_(\\d+)\\/?$/i)||[])[1],scope=(u.pathname.match(/\\/sc_(shinagawa|meguro)\\//i)||[])[1];if(id){scope=scope||(x.ward==='目黒区'?'meguro':'shinagawa');return`https://suumo.jp/chukoikkodate/tokyo/sc_${scope}/nc_${id}/`}}catch(_){let id=(raw.match(/nc_(\\d+)/i)||[])[1];if(id)return`https://suumo.jp/chukoikkodate/tokyo/sc_${x.ward==='目黒区'?'meguro':'shinagawa'}/nc_${id}/`}return raw.replace(/[?#].*$/,'').replace(/\\/+$/,'')+'/'}return raw}function linkHtml(x,label='物件を開く'){let url=listingUrl(x);return url?`<a class=\"open-btn\" href=\"${esc(url)}\" target=\"_blank\" rel=\"noopener noreferrer\" onclick=\"event.stopPropagation()\">${label}</a>`:'<span class=\"muted\">リンクなし</span>'}"
if old_link in source:
    source = source.replace(old_link, new_link, 1)
elif malformed_link in source:
    source = source.replace(malformed_link, new_link, 1)
elif new_link not in source:
    raise SystemExit("Could not locate the v1 linkHtml function")
